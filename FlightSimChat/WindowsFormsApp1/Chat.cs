...
namespace WindowsFormsApp1
{
    ...
    WHERE @currentUserId IN (m.sender_id, m.receiver_id)";
                    ...
                    {
                        cmd.Parameters.AddWithValue("@currentUserId", CurrentUser.UserId); // Replace with the logged-in user's ID
                ...
                ORDER BY date_sent ASC";
                    ...
                    using (SqlCommand cmd = new SqlCommand(query, conn))
                    {
                        ...
                        using (SqlDataReader reader = cmd.ExecuteReader())
                        {
                            ...
                            while (reader.Read())
                            {
                                string sender = (int)reader["sender_id"] == CurrentUser.UserId ? "You" : "User";
        ...
        private void SendMessage(int receiverId, string messageContent)
        {
            ...
            try
            {
                ...
                VALUES (@senderId, @receiverId, @content, @dateSent, 'Sent')";
                    ...
                    using (SqlCommand cmd = new SqlCommand(query, conn))
                    {
                        cmd.Parameters.AddWithValue("@senderId", CurrentUser.UserId); // Replace with the logged-in user's ID
